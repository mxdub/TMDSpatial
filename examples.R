...
profvis({
  simulate_MC <- function(patches, species, dispersal = 0.01,
    ...
    for(i in 1:(initialization + burn_in + timesteps)){
      ...
      #                     data.frame(N = c(N), patch = 1:patches, species = rep(1:species, each = patches), env = env, time = i-initialization-burn_in))
      setTxtProgressBar(pb, i)
    }
    close(pb)
    dynamics.df = do.call(rbind, dynamics.df) # new line
